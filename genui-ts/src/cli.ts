...
选项:
  -v, --version          显示版本号
  -h, --help             显示帮助信息
  -a, --adapter <type>   选择适配器 (web|terminal|test), 默认: test
...
function parseCliArgs(): { version: boolean; help: boolean; adapter: AdapterType } {
  const { values } = parseArgs({
    options: {
      ...
      adapter: { type: 'string', short: 'a', default: 'test' },
    },
    strict: true,
  });
...
function createAdapter(adapterType: AdapterType): UIAdapter {
  switch (adapterType) {
    ...
    case 'web':
      console.error('错误: web适配器尚未实现, 请使用 --adapter test');
      process.exit(1);
      break; // eslint: unreachable but keeps TS happy after noreturn
    case 'terminal':
      console.error('错误: terminal适配器尚未实现, 请使用 --adapter test');
      process.exit(1);
      break;
    default:
